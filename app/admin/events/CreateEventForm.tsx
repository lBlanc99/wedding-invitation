...
export default function CreateEventForm() {
  ...
  return (
    <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200 mb-10">
        <h2 className="font-bold text-lg text-slate-900 mb-6 border-b border-slate-100 pb-4">Create New Event</h2>
        
        <form ref={formRef} action={handleSubmit} className="flex flex-col gap-5">
          
          <div>
            <label className="text-[10px] font-bold text-slate-500 uppercase mb-1 block tracking-wider">Event Name</label>
            <input name="name" placeholder="Example: Ricky & Angela Wedding" className="w-full border border-slate-300 p-3 rounded-xl text-sm text-slate-900 bg-white placeholder:text-slate-400 focus:ring-2 focus:ring-indigo-500 outline-none font-medium" required />
          </div>

          <div>
            <label className="text-[10px] font-bold text-slate-500 uppercase mb-1 block tracking-wider">Event Link / Code (Optional)</label>
            <div className="flex">
            ...
            <p className="text-[10px] text-slate-400 mt-1.5">*Leave empty to auto-generate.</p>
          </div>

          {/* DUAL LOCATION INPUTS */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              <label className="text-[10px] font-bold text-indigo-600 uppercase mb-1 block tracking-wider">Matrimony Location</label>
              <input name="matrimonyLocation" placeholder="Example: Cathedral Church" className="w-full border border-slate-300 p-3 rounded-xl text-sm text-slate-900 bg-white placeholder:text-slate-400 focus:ring-2 focus:ring-indigo-500 outline-none" required />
            </div>
            <div>
              <label className="text-[10px] font-bold text-pink-600 uppercase mb-1 block tracking-wider">Reception Location</label>
              <input name="receptionLocation" placeholder="Example: Majapahit Hotel" className="w-full border border-slate-300 p-3 rounded-xl text-sm text-slate-900 bg-white placeholder:text-slate-400 focus:ring-2 focus:ring-indigo-500 outline-none" required />
            </div>
          </div>
