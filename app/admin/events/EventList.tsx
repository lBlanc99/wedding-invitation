...
export default function EventList({ events }: { events: Event[] }) {
  ...
  async function handleDelete(id: number) {
    const result = await Swal.fire({
      ...
      text: "Semua data tamu & foto di dalamnya akan hilang permanen!",
      icon: 'warning',
        ...
        {events.map((event) => (
          <div key={event.id} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 hover:shadow-md transition group relative">
            ...
            <button 
              ...
              className="absolute top-4 right-4 text-slate-300 hover:text-red-500 transition p-2 hover:bg-red-50 rounded-full"
              title="Hapus Event"
            >
            ...
            <div className="mb-4">
               <span className="bg-slate-100 text-slate-600 px-3 py-1 rounded-full text-xs font-bold border border-slate-200">
                {event._count.guests} Tamu Terdaftar
              </span>
            </div>
            ...
            <div className="bg-slate-50 rounded-xl p-4 border border-slate-100 space-y-3">
              <div className="flex justify-between items-center">
                ...
                <span className="text-xs font-bold text-slate-900">
                  {new Date(event.matrimonyDate).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' })}, {new Date(event.matrimonyDate).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })}
                </span>
              </div>
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="text-xs font-bold text-slate-900">
                  {new Date(event.receptionDate).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' })}, {new Date(event.receptionDate).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })}
                </span>
              </div>
            </div>
