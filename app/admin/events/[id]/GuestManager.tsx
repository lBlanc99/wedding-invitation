...
import QRCode from 'qrcode' // <--- Import Library QR
...
export default function GuestManager({ guests, eventId }: { guests: Guest[], eventId: number }) {
  ...
  function handleCopyLink(code: string) {
    // Otomatis ambil domain (localhost atau website asli)
  ...
  async function handleDownloadQR(code: string, name: string) {
    try {
      ...
      // Generate QR ke Data URL (Base64 Image)
      ...
      // Buat element <a> palsu untuk trigger download
      const link = document.createElement('a')
      link.href = qrDataUrl
      link.download = `QR_${name.replace(/\s+/g, '_')}.png` // Nama file: QR_Nama_Tamu.png
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left">
          <thead className="bg-slate-50 text-slate-500 uppercase text-[10px] tracking-wider font-bold border-b border-slate-200">
            <tr><th className="p-4 w-[25%]">Nama Tamu</th><th className="p-4 w-[35%]">Link & QR</th><th className="p-4 w-[15%]">Kategori</th><th className="p-4 w-[15%]">Akses</th><th className="p-4 w-[10%] text-right">Aksi</th></tr>
          </thead>
          <tbody className="divide-y divide-slate-100 text-sm">
            {filteredGuests.map((guest) => (
              <tr key={guest.id} className="hover:bg-slate-50 transition group">
                ...
                {/* KOLOM BARU: LINK & TOOLS */}
                <td className="p-4">
                  <div className="flex items-center gap-2">
                    {/* Link Text */}
                    <a href={`/${guest.code}`} target="_blank" className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-slate-100 text-slate-700 border border-slate-200 hover:bg-slate-200 transition font-mono text-xs font-bold">
                       /{guest.code}
                       <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-3 h-3 text-slate-400"><path fillRule="evenodd" d="M4.25 5.5a.75.75 0 00-.75.75v8.5c0 .414.336.75.75.75h8.5a.75.75 0 00.75-.75v-4a.75.75 0 011.5 0v4A2.25 2.25 0 0112.75 19h-8.5A2.25 2.25 0 012 16.75v-8.5A2.25 2.25 0 014.25 6h4a.75.75 0 010 1.5h-4z" clipRule="evenodd" /><path fillRule="evenodd" d="M6.194 12.753a.75.75 0 001.06.053L16.5 4.44v2.81a.75.75 0 001.5 0v-4.5a.75.75 0 00-.75-.75h-4.5a.75.75 0 000 1.5h2.81l-8.39 8.363a.75.75 0 00-.053 1.06z" clipRule="evenodd" /></svg>
                    </a>

                    {/* Tombol Copy */}
                    <button 
                      onClick={() => handleCopyLink(guest.code)}
                      className="p-1.5 bg-white border border-slate-200 rounded-md text-slate-500 hover:text-indigo-600 hover:border-indigo-200 shadow-sm transition" 
                      title="Copy Link"
                    >
                    ...
                    {/* Tombol QR */}
                    <button 
                      onClick={() => handleDownloadQR(guest.code, guest.name)}
                      className="p-1.5 bg-white border border-slate-200 rounded-md text-slate-500 hover:text-indigo-600 hover:border-indigo-200 shadow-sm transition" 
                      title="Download QR"
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4">
            ...
            {filteredGuests.length === 0 && <tr><td colSpan={5} className="p-8 text-center text-slate-400 italic bg-slate-50/50">Tidak ada tamu yang sesuai filter.</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
