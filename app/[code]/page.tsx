...
import GuestQRCode from './GuestQRCode' // <--- 1. Import Component Baru
...
function GuestInvitation({ guest, messages }: { guest: any, messages: any[] }) {
  ...
  return (
    <main className="min-h-screen bg-[#FDFBF7] text-[#4A4A4A] overflow-hidden relative font-sans">
       ...
       {/* === SECTION QR CODE (BARU) === */}
      <section className="max-w-md mx-auto px-6 mb-20 relative z-10">
        {/* Update props disini: */}
        <GuestQRCode guestName={guest.name} guestCode={guest.code} />
      </section>
...
// ... (sisanya EventLanding dan Page export default TETAP SAMA) ...
    ...
    <main className="min-h-screen bg-[#FDFBF7] flex flex-col items-center justify-center p-6 relative overflow-hidden font-sans">
      ...
      <div className="relative z-10 w-full max-w-md bg-white/70 backdrop-blur-xl border border-white/60 p-10 rounded-[40px] text-center shadow-2xl">
        ...
        <form action={checkGuestCode} className="flex flex-col gap-5">
          <input name="guestCode" className="w-full bg-white/80 border-2 border-[#E5E0D8] rounded-2xl p-4 text-center text-2xl tracking-[0.4em] font-serif text-[#2C2C2C] focus:outline-none focus:border-[#D4AF37] shadow-inner uppercase" placeholder="XXXXXX" required autoComplete="off" />
          <button className="bg-gradient-to-r from-[#2C2C2C] to-[#4a4a4a] hover:from-[#1a1a1a] hover:to-[#333] text-[#D4AF37] font-sans text-xs font-bold uppercase tracking-[0.2em] py-5 rounded-2xl transition shadow-xl mt-2 border-b-4 border-[#D4AF37]/30 active:border-b-0 active:translate-y-1">Open Invitation</button>
        </form>
      </div>
    </main>
  )
